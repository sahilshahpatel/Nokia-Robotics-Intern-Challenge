...
void Fsm_Run ()
{
    ...
    Serial.print("FSM_STATE_NEXT_TARGET\n");
		Turret_Set_Angle(0);
		Nav_Set_Target2(NAV_COORD,100, drive_pos_x, drive_pos_y+2, 1);
    fsm_state = FSM_STATE_WAIT_NAV;
   ...
   Serial.print("FSM_STATE_TURN_RIGHT_WALL\n");
    Nav_Set_Target2(NAV_HEAD, 100, 0, 0.0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    Turret_Set_Angle (-90);
    Turn_param = 1;
    ...
    Nav_Set_Target2(NAV_HEAD, 100, 180, 0.0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    Turret_Set_Angle (90);
    Turn_param = 0;
    fsm_next_state = FSM_STATE_SEARCH;
    ...
    Nav_Set_Target2(NAV_FORWARD, 100, 17, 0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    fsm_next_state = FSM_STATE_NEXT_TARGET;
    break;
    ...
    //Nav_Set_Target2(NAV_FORWARD, 100, 50, 0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    if(Turn_param == 1){
      Nav_Set_Target2(NAV_COORD, 100, drive_pos_x + 2, drive_pos_y, 1);
    }
    if(Turn_param == 0){
      Nav_Set_Target2(NAV_COORD, 100, drive_pos_x - 2, drive_pos_y, 1);
    } 
    ...
    case FSM_STATE_BACK_SEARCH_PREP:
    Drive_Set_Speed(0,0);
    fsm_state = FSM_STATE_WAIT_MICROS;
    fsm_micros_timeout = micros() + 500000;
    ...
    if(backwards_param == 1){
      Nav_Set_Target2(NAV_COORD, -50, drive_pos_x-2, drive_pos_y+0, 1);
      fsm_state = FSM_STATE_WAIT_NAV;
      if(ping_dist[0] >= 25){
    ...
    Turret_Set_Angle(0);
    Nav_Set_Target2(NAV_FORWARD,100, 1, 0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    if(ping_dist[0] < 12 && ping_dist[0] > 0){
