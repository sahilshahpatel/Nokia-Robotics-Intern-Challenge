...
void Fsm_Run ()
{
  ...
  switch (fsm_state) {
    ...
    fsm_state = FSM_STATE_TURN_WEST;
    break;

    case FSM_STATE_TURN_WEST:
    Nav_Set_Target2(NAV_HEAD, 90, 180, 0.0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    fsm_next_state = FSM_STATE_MOVE_WEST;
    break;

    case FSM_STATE_MOVE_WEST:
    Nav_Set_Target2(NAV_COORD, 90, drive_pos_x - 5, drive_pos_y, 2);
    fsm_state = FSM_STATE_WAIT_NAV;
    fsm_next_state = FSM_STATE_TURN_NORTH;
    break;

    case FSM_STATE_TURN_NORTH:
    Turret_Set_Angle(-90);
    ...
    if(ping_dist[0]<=15){
      reflector_1 = 1;
      Serial.print(reflector_1);
    }
    if(drive_pos_y <= -20){
      fsm_next_state = FSM_STATE_SCAN_SECOND;
    }
    else{
      fsm_next_state = FSM_STATE_PREP_SCAN_FIRST;
    }
    ...
    case FSM_STATE_SCAN_SECOND:
    Serial.print(drive_pos_y);
    Nav_Set_Target2(NAV_COORD, -90, drive_pos_x, drive_pos_y-2, 1);
    fsm_state = FSM_STATE_WAIT_NAV;
    if(ping_dist[0]<=15){
      reflector_2 = 1;
      Serial.print(reflector_2);
    }
    if(drive_pos_y <= -40){
    ...
    Nav_Set_Target2(NAV_HEAD, 90, 0, 0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    fsm_next_state = FSM_STATE_MOVE_EAST;
    break;

    case FSM_STATE_MOVE_EAST:
    Nav_Set_Target2(NAV_COORD, 90, drive_pos_x + 10, drive_pos_y, 2);
    fsm_state = FSM_STATE_WAIT_NAV;
    fsm_next_state = FSM_STATE_TURN_SOUTH;
    break;
    
    case FSM_STATE_TURN_SOUTH:
    Nav_Set_Target2(NAV_HEAD, 90, 290, 0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    ...
    if(drive_pos_y >= 0){
      fsm_next_state = FSM_STATE_MOVE_SOUTH;
    }
    else{
      fsm_next_state = FSM_STATE_SCAN_FOURTH;
    }
    ...
    case FSM_STATE_MOVE_SOUTH:
    Nav_Set_Target2(NAV_COORD, 90, drive_pos_x, -40, 1);
    fsm_state = FSM_STATE_WAIT_NAV;
    if(drive_pos_y <= -40){
    ...
    if(total == 1){
      Turret_Set_Angle(-45);
      Nav_Set_Target2(NAV_COORD, 90, -8, -46, 1);
      fsm_state = FSM_STATE_WAIT_NAV;
      fsm_next_state = FSM_STATE_DONE;
    }
    if(total == 2){
      Turret_Set_Angle(0);
      Nav_Set_Target2(NAV_COORD, 90, 0, -46, 1);
      fsm_state = FSM_STATE_WAIT_NAV;
      fsm_next_state = FSM_STATE_DONE;
    }
    if(total == 3){
    ...
    if(total == 4){
      Turret_Set_Angle(90);
      Nav_Set_Target2(NAV_COORD, 90, 16, -46, 1);
      fsm_state = FSM_STATE_WAIT_NAV;
      fsm_next_state = FSM_STATE_DONE;
    }
    if(total == 0){
