...
enum	ping_state_enum {
	PING_STATE_INITFAIL1	= 127,	// Init fail (check hardware)
...
int8_t  Turn_param;
int8_t  starting_left_dist;
int8_t  starting_right_dist;
int8_t  first_right_dist;
int8_t  ctr = 0;
...
enum	fsm_state_enum {
	...
	FSM_STATE_TURN_RIGHT_WALL,		// Set turret to -90 and ping 1 second
  FSM_STATE_TURN_LEFT_WALL,
  FSM_STATE_TURN_RIGHT_GAP,
  FSM_STATE_TURN_LEFT_GAP,
  FSM_STATE_CONT_NORTH,
  FSM_STATE_NORTH_UKNOWN,
  FSM_STATE_SEARCH,
  FSM_STATE_SCAN_LEFT,
  FSM_STATE_SCAN_RIGHT,
  FSM_STATE_SCAN_TEMP,
  FSM_STATE_SCAN_LEFT_AGAIN,
  FSM_STATE_SCAN_RIGHT_AGAIN,
  FSM_STATE_NORTH_UNKNOWN,
  FSM_STATE_PREP_SCAN,
...
void Fsm_Run ()
{
    ...
    starting_left_dist = ping_dist[0]; 
    fsm_state = FSM_STATE_WAIT_MICROS;
    fsm_micros_timeout = micros() + 1000000;
    fsm_next_state = FSM_STATE_SCAN_RIGHT;
    break;

    case FSM_STATE_SCAN_RIGHT:
    Serial.print("FSM_STATE_SCAN_RIGHT\n");
    Turret_Set_Angle (90);
    fsm_state = FSM_STATE_WAIT_MICROS;
    fsm_micros_timeout = micros() + 1000000;
    starting_left_dist = ping_dist[0]; 
    if(ctr == 0){
      first_left_dist = ping_dist[0]; 
    }
    fsm_state = FSM_STATE_WAIT_MICROS;
    fsm_micros_timeout = micros() + 1000000;
    fsm_next_state = FSM_STATE_SCAN_TEMP;
    break;

    case FSM_STATE_SCAN_TEMP:
    Serial.print("FSM_STATE_SCAN_TEMP\n");
    starting_right_dist = ping_dist[0];
    if(ctr == 0){
      first_right_dist = ping_dist[0];
      ctr += 1;
    }
    fsm_state = FSM_STATE_WAIT_MICROS;
    fsm_micros_timeout = micros() + 1000000;
    ...
    Serial.print("FSM_STATE_NEXT_TARGET\n");
		Turret_Set_Angle(0);
		Nav_Set_Target2(NAV_COORD,90, drive_pos_x, drive_pos_y+2, 1);
    fsm_state = FSM_STATE_WAIT_NAV;
    //Serial.print(starting_left_dist);
    //Serial.print("\n");
    //Serial.print(starting_right_dist);
    if(ping_dist[0] < 7 && ping_dist[0] > 0){
      if(starting_right_dist > starting_left_dist){
        fsm_next_state = FSM_STATE_TURN_RIGHT_WALL;
      }
      else{
        fsm_next_state = FSM_STATE_TURN_LEFT_WALL;
      }
    }
    else{
      fsm_next_state = FSM_STATE_NEXT_TARGET;
	  ...
	  case FSM_STATE_TURN_RIGHT_WALL:
   Serial.print("FSM_STATE_TURN_RIGHT_WALL\n");
    Nav_Set_Target2(NAV_HEAD, 90, 0, 0.0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    Turret_Set_Angle (-90);
    Turn_param = 1;
    fsm_next_state = FSM_STATE_SEARCH;
		break;

    case FSM_STATE_TURN_LEFT_WALL:
    Serial.print("FSM_STATE_TURN_LEFT_WALL\n");
    Nav_Set_Target2(NAV_HEAD, 90, 180, 0.0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    Turret_Set_Angle (90);
    Turn_param = 0;
    fsm_next_state = FSM_STATE_SEARCH;
    ...
    case FSM_STATE_TURN_RIGHT_GAP:
    Serial.print("FSM_STATE_TURN_RIGHT_GAP\n");
    Nav_Set_Target2(NAV_HEAD, 90, 90, 0.0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    Turret_Set_Angle (0);
    Turn_param = 1;
    ...
    case FSM_STATE_TURN_LEFT_GAP:
    Serial.print("FSM_STATE_TURN_LEFT_GAP\n");
    Nav_Set_Target2(NAV_HEAD, 90, 90, 0.0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    Turret_Set_Angle (0);
    Turn_param = 0;
    ...
    Serial.print("FSM_STATE_PREP_SCAN\n");
    Nav_Set_Target2(NAV_FORWARD, 90, 13, 0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    fsm_next_state = FSM_STATE_SCAN_LEFT;
    break;

    case FSM_STATE_SEARCH:
    Serial.print("FSM_STATE_SEARCH\n");
    
    //Nav_Set_Target2(NAV_FORWARD, 50, 50, 0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    if(Turn_param == 1){
      Nav_Set_Target2(NAV_COORD, 90, drive_pos_x + 2, drive_pos_y, 1);
    }
    if(Turn_param == 0){
      Nav_Set_Target2(NAV_COORD, 90, drive_pos_x - 2, drive_pos_y, 1);
    } 
    ...
    //fsm_next_state = FSM_STATE_CHOOSE;
    Serial.print(ping_dist[0]);
    Serial.print("\n");
    if(ping_dist[0] >= 20 && Turn_param == 0){
      fsm_next_state = FSM_STATE_TURN_RIGHT_GAP;
    }
    else if(ping_dist[0]>= 20 && Turn_param == 1){
      fsm_next_state = FSM_STATE_TURN_LEFT_GAP;
    }
    else{
      fsm_next_state = FSM_STATE_SEARCH;
    }

    Serial.print("First_right_dist: "); 
    Serial.print(first_right_dist);
    Serial.print("\n");
    Serial.print("First_left_dist: ");
    Serial.print(first_left_dist);
    Serial.print("\n");
    ...
    if(drive_pos_x > first_right_dist - 7){
      fsm_next_state = FSM_STATE_BACK_SEARCH;
      backwards_param = 1; // on east side
    }
    if(drive_pos_x < -1*(first_left_dist - 7)){
      fsm_next_state = FSM_STATE_BACK_SEARCH; 
      backwards_param = 0; // on west side
    }
    break;

    case FSM_STATE_BACK_SEARCH:
    if(backwards_param == 1){
      Nav_Set_Target2(NAV_COORD, -50, drive_pos_x-2, drive_pos_y, 1);
      fsm_state = FSM_STATE_WAIT_NAV;
      if(ping_dist[0] >= 20){
        Nav_Set_Target2(NAV_FORWARD, -50, -2, 0,0);
        fsm_state = FSM_STATE_WAIT_NAV;
        fsm_next_state = FSM_STATE_TURN_LEFT_GAP;
      }
      else{
    ...
    case FSM_STATE_CONT_NORTH:
    Turret_Set_Angle(0);
    Nav_Set_Target2(NAV_FORWARD,50, 1, 0, 0);
    fsm_state = FSM_STATE_WAIT_NAV;
    if(ping_dist[0] < 10 && ping_dist[0] > 0){
      ...
      fsm_next_state = FSM_STATE_CONT_NORTH;
    }
    break;
